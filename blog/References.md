...
understanding the content. We will follow the example in the README 
https://github.com/advancedtelematic/quickcheck-state-machine/blob/master/README.md, so familiarity with this helps.
Also this has a more advanced example http://www.well-typed.com/blog/2019/01/qsm-in-depth/.
...
q-s-m https://github.com/advancedtelematic/quickcheck-state-machine/blob/master/README.md, how to we generate a Command 
to write to an IORef, even before it exists?
...
```
as we can see Concrete are simple wrappers of the real a. As you may have noticed we say that the user defined
Commands and Responses contain references (Symbolic or Concrete), that is they have the structure of a container.
Indeed, for the generation of commands, q-s-m requires a Foldable instance of Responses, so that it can extract the list of 
Variables (like the [ Var 0 ] we saw above. The Symbolic responses are created by a user defined functions:
``` haskell
mock           :: model Symbolic -> cmd Symbolic -> GenSym (resp Symbolic)
```
The GenSym monad is a state monad, where state is a counter.
Execution requires more constraints and here is the part where the concrete Response and Command is created. 
Foldable Response is still needed, because we want to extract the list of Concrete
References this time. By assuming that the extracted References are on the same order, the execution keeps track of the
environment: that is a simple `Data.Map` of how Symbolic References map to Concrete ones. This Environment helps reifying
symbolic commands to concrete ones. To achieve this, execution traverse through the References of the Symbolic Commands and 
...
instance of Commands is necessary. 
...
The user can provide a shrinker for a single Commands, but q-s-m also tries to shrink the list of Commands, by discarding
whole commands. This is tricky though, because we must make sure that the references of the discarded Commands are not used.
...
Initially this counter is zero and the Map empty. So when the validate logic runs mock for this command:
``` haskell
Command Create [ Var 1 ]
```
for counter=0 and extra the Variables of the response, it will be [ Var 0 ]. A mapping 0 -> 1 is kept in the environment
for future commands. Next Commands are traversed and remmaped, so the final commands will look like:
...
```
This traversal requires a Traversable instance for the commands, which as we mentioned is already needed for execution,
...
ParallelCommands
  ...
  ```
This example is pretty simple because the first thread actually executed no commands, so it's not actually parallel.
...
This bug was an open issue in q-s-m https://github.com/advancedtelematic/quickcheck-state-machine/issues/302 and it apppeared
from time to time in ci and threw a strange error which confused users. It was fixed with my pr 
https://github.com/advancedtelematic/quickcheck-state-machine/pull/315. This bugged also affected me while 
trying to generalize parallelism in q-s-m: In this pr 
https://github.com/advancedtelematic/quickcheck-state-machine/pull/324 I extended parallelism from using strictly 2
threads, to using a user specified amount of threads. So insted of having a Pair, I used a list. Before merging this
pr I was trying to add a sanity test, which tested the equivalence of the old implementation with the new one for n=2.
The tests I had added kept failing and it was only when this bug was fixed that the tests passed. This thread has some
more details about this https://github.com/advancedtelematic/quickcheck-state-machine/pull/294#discussion_r272949230.

Another bug related to references that I fixed, did not have to do with shrinking, but with generation and execution.
...
Assume a Model of a system where we can create only create references:
``` haskell
data Command r
  = Create
```
...
fix made programs like the above to be discarded. The fix was added in this pr 
https://github.com/advancedtelematic/quickcheck-state-machine/pull/348.
